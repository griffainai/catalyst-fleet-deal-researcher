// Mirrors the researcher folder (one level up) into ./.researcher so the
// deployed serverless function is self-contained. Runs on predev + prebuild.
//
// Why: the API route feeds the researcher's markdown to Claude as the system
...
const SRC = path.resolve(PLAYROOM, ".."); // the researcher folder
...
async function sourcePresent() {
  ...
  // build). On a playroom-only deploy the committed .researcher is the source
  // of truth — we must NOT wipe it.
  try {
...
async function main() {
  if (!(await sourcePresent())) {
    console.log(
      "[sync-researcher] source folder not present — keeping the committed .researcher/ mirror as-is."
    );
    return;
  }
  ...
  console.log(`[sync-researcher] mirrored ${count} files into .researcher/`);
}
