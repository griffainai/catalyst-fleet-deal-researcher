...
import type { AngleId } from "@/lib/brand";
...
export function useResearchStream() {
  ...
  const send = useCallback(
    async (text: string, angle: AngleId | "rank" | "duel", history?: ChatMessage[]) => {
      ...
      try {
        const res = await fetch("/api/research", {
          ...
          body: JSON.stringify({ messages: updated, angle }),
          signal: ctrl.signal,
        });

        if (!res.ok || !res.body) {
          const errBody = await res.json().catch(() => ({ error: "Unknown error" }));
          setError(errBody.error || "Researcher API failed");
          setMessages(updated);
          setIsStreaming(false);
          return;
        }
